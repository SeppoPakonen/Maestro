...
class KotlinParser(TranslationUnitParser):
    ...
    def parse_file(self, path: str, *, compile_flags: Optional[Sequence[str]] = None, verbose: bool = False) -> ASTDocument:
        ...
        try:
            ...
            raise ParserExecutionError(f"Failed to parse Kotlin file: {exc}") from exc
    ...
    def _extract_name_from_node(self, ts_node, source_bytes: bytes) -> str:
        ...
        # For Kotlin-specific node types that have names
        if ts_node.type in ['simple_identifier', 'type_identifier', 'class_declaration',
                           'function_declaration', 'property_declaration', 'variable_declaration',
                           'object_declaration', 'interface_declaration', 'enum_class']:
            for child in ts_node.children:
                if child.type == 'simple_identifier':
                    return _get_node_text(child, source_bytes)

        # For class declarations with an identifier child
        if ts_node.type in ['class_declaration', 'interface_declaration', 'object_declaration']:
            for child in ts_node.children:
                if child.type == 'identifier' or child.type == 'simple_identifier':
                    return _get_node_text(child, source_bytes)
        ...
        if ts_node.type in ['property_declaration', 'variable_declaration']:
            for child in ts_node.children:
                if child.type == 'simple_identifier':
                    return _get_node_text(child, source_bytes)

        # For method calls and similar that have a simple_identifier child
        if ts_node.type in ['method_invocation', 'call_expression', 'field_access']:
            for child in ts_node.children:
                if child.type == 'simple_identifier':
                    return _get_node_text(child, source_bytes)
    ...
    def _maybe_add_symbol(self, ts_node, loc: SourceLocation, kind_str: str, symbols: List[Symbol], source_bytes: bytes) -> None:
        ...
        # Map tree-sitter Kotlin node types to semantic types
        if ts_node.type in [
            'class_declaration', 'interface_declaration', 'object_declaration', 'enum_class',
            'function_declaration', 'property_declaration', 'variable_declaration'
        ]:
            name = self._extract_name_from_node(ts_node, source_bytes)

            # For property and variable declarations, get type information from the parent
            type_info = None
            if ts_node.type in ['property_declaration', 'variable_declaration']:
        ...
        elif ts_node.type in ['call_expression', 'simple_identifier', 'field_access']:
            name = self._extract_name_from_node(ts_node, source_bytes)
            if name:
