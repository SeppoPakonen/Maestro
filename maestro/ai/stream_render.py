...
class StreamRenderer:
    ...
    def render_event(self, event: AiStreamEvent):
        ...
        # Handle Qwen specially - in normal mode, only show assistant text
        if self.engine == "qwen":
            if event.type == EventType.MESSAGE and event.text_full and not self.verbose:
                print(event.text_full, end="", flush=True)
                self.accumulated_text += event.text_full
            elif self.verbose:
                # In verbose mode, show the parsed event
                self._render_verbose_event(event)
        else:
            # For other engines, use the default behavior
