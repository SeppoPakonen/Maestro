...
def write_feedback(state_dir: str, cookie: str, feedback: dict, redaction_rules=None) -> str:
    """
    ...
    # Apply redaction to feedback if rules are provided
    # For simplicity, we'll serialize to YAML string, apply redaction, then write
    if redaction_rules:
        # Serialize to YAML string
        yaml_str = yaml.dump(feedback_out, default_flow_style=False, allow_unicode=True)
        # Apply redaction
        redacted_yaml_str = redact_text(yaml_str, redaction_rules)
        ...
        with open(feedback_file_path, "w", encoding="utf-8") as f:
            f.write(redacted_yaml_str)
    else:
        # Write to YAML file without redaction
        feedback_file_path = os.path.join(feedback_dir, f"{cookie}.yaml")
        with open(feedback_file_path, "w", encoding="utf-8") as f:
